	...
	return class Collator {
		...
		collate(token, tokens) {
			...
			if (this.node(token) === undefined) {
							...
							if ('{' === token.text) {
								if (constructNode[symbols.BlockConstruct] === null) {
									...
									if (constructNode[symbols.BindingConstruct] === null) {
										constructNode[symbols.BindingConstruct] = currentNode;
									} else if (constructNode[symbols.ClassBody] === null) {
										constructNode[symbols.ClassBody] = currentNode;
									} else if (
										(constructNode = currentNode[symbols.FunctionConstruct] =
											constructNode[symbols.FunctionConstruct]) != null &&
										constructNode[symbols.FunctionBody] === null
									) {
										constructNode[symbols.BlockConstruct] = currentNode;
										currentNode[symbols.FunctionArguments] = constructNode[symbols.ArgumentConstruct];
										constructNode[symbols.FunctionBody] = currentNode;
										constructNode[symbols.FunctionBody][symbols.FunctionBody] = currentNode;
									} else {
										debugger;
									}
								}
				...
				// this.setNodeContext((this.lastContext = tokenContext), (this.lastNode = node));
