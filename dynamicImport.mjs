export default async function dynamicImport(specifier, referrer) {
	if (!('resolve' in dynamicImport)) {
		...
		let base;
		const resolve = freeze((specifier, referrer) => `${new URL(specifier, referrer || base || import.meta.url)}`);
		...
		try {
			...
			{
				const global = (typeof globalThis === 'object' && globalThis) || (1, eval)('this');
				!global || 'dynamicImport' in global || (global.dynamicImport = dynamicImport);
			}
		}
