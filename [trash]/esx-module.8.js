console.clear();
setTimeout(async (LEVEL = 3, CYCLES) => {
  /// ESX Modules Experiment

  // const ModuleScope = getModuleScope();

  with (getModuleScope({LEVEL, CYCLES})) {
...
function getModuleScope(SWITCHES) {
  ...
  class Module {
    ...
    static instantiate(module) {
      ...
      //   define(bindings, 'module', context, false, true);
      //   const scope = setPrototypeOf(bindings, ModuleScope);
      //   // log(Object.getOwnPropertyNames(bindings));
      //   define(context, 'scope', scope, ENUMERABLE, false);
      //   return freeze(context), scope;
      // };
      // bind(context, 'scope', initialize, ENUMERABLE, true);
      // define(context, 'module', context, ENUMERABLE, false);
