    ...
    if (LEVEL >= 4) {
      new Module('circular-imports-1', async module => {
        ...
        with (module.scope) {
          ...
          let a, b, c;
          a = `${(typeof a1 === 'string' && `${a1} `) || ''}a${n}`;
          await module.export(() => a);
          b = `${(typeof b1 === 'string' && `${b1} `) || ''}b${n}`;
          await module.export(() => b);
          c = `${(typeof c1 === 'string' && `${c1} `) || ''}c${n}`;
...
function getModuleScope({LEVEL = 3} = {}) {
  ...
  class Module {
    ...
    link() {
      ...
      const promises = [];
      const imports = {};
      const {namespaces, bindings, links} = this;
      ...
      for (const binding in links) {
        ...
        // const imported = namespaces.import(url);
        const imported =
          imports[url] ||
          (imports[url] =
        ...
        if (intent === 'import') {
          ...
          imported.then(() => {
            context.export || Module.instantiate(this);
    ...
    instantiate() {
      ...
      //     try {
      //       // TODO: Premature copy may need to be bound instead!
    ...
    static instantiate(module) {
      ...
      // define(module, 'context', Object.create(null), false);
      context.export = (...exports) => void Module.bind(namespace, ...exports);
      context.export.from = (...links) => {
        for (const link of links) {
          const {intent, specifier, identifier, binding, url} = link;
          intent === 'export' && copy(namespace, namespaces[url], identifier, binding); // bind(namespace, binding, () => namespaces[url][identifier], ENUMERABLE, false);
        }
      };
      context.export.default = value => void Module.bind(namespace, {default: () => value});
      const initialize = () => {
